...
<div id="contenido">
    <div style="position:absolute; right:10px"> POR: <b> <u>$ {{$contrato->pago}}</u></b> </div>
    <h5>
        RECIBÍ DE LA UNIVERSIDAD CATÓLICA DE EL SALVADOR, LA CANTIDAD DE:
    </h5>
    <b><u>
        {{$contrato->cantidadEnLetras}} 
        dólares
    </u></b>
    <br>
    <br>
    ...
    <p>ILOBASCO, <u>{{$dia}}</u> de <u>{{$mes}}</u> de {{$ano}}</p>
    <br>
    <br>
    ...
    <div style="margin:0px">
        ...
        <div class="right">
                Devengado  = <b > <u>$ {{$contrato->pago}}</u></b>
            <br>
                @if($consultor->empresa)
                    ...
                    ?>
                @else
                    ISR = $ <u>{{ $descuento = number_format(($contrato->pago / 1.13) * 0.1 , 2)}}</u>
                @endif
            <br>
                A PAGAR = <b ><u>$ {{number_format($contrato->pago - $descuento, 2)}}</u></b>
            <br>
        </div>
    </div>
</div>
