...
<body>
    <div class="container">
        ...
        <hr>
        <div class="container">
            @if (Carbon\Carbon::parse($cap->fecha_limite)->format('d-m-Y') <= date('d-m-Y'))
            <div class="row">
                <div class="col-sm-8 col-sm-offset-2">
                    ...
                    @else
                    <form method="POST" action="{{ route('guardarOfertaCap') }}" enctype="multipart/form-data">
                        {!! csrf_field() !!}
                        {{-- <input type="hidden" name="id" value="{{ $oferta->id }}"> --}}
                        <input type="hidden" name="consultor_id" value="{{ $consultor->code }}">
                        <input type="hidden" name="cap_id" value="{{ $cap->code }}">
                        <h2>Hola, {{ $consultor->nombre }}!</h2>
                        <p>Aca puedes subir tu oferta para la Capacitación "{{ $cap->tema }}".</p>
                        <br>
                        <p>Sube la oferta antes del {{ Carbon\Carbon::parse($cap->fecha_limite)->format('d-m-Y') }}.</p>
                        <br>
            ...
            <div class="row">
                <div class="col-sm-8 col-sm-offset-2">
                    <h2>Hola, {{ $consultor->nombre }}!</h2>
                    <p>Lamentamos que ya no puedas subir tu oferta, pero la fecha límite para hacerlo era el {{ Carbon\Carbon::parse($cap->fecha)->format('d-m-Y') }}.</p>
                </div>
            </div>     
            @endif
        </div>
